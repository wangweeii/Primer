...
public:
        enum Status
        {
                critical, environment, improving
        };
...
public:
        enum DanceType
        {
                two_left_feet, macarena, fandango
        };
...
Panda::Panda(std::string name, bool onExhibit) : ZooAnimal(name, onExhibit, "Panda"), Bear(name, onExhibit), Raccoon(name, onExhibit), Endangered(
        Endangered::critical), sleeping_flag(false)
{}
